...
sinAceitunas pizza = returnSomethingAboutPizza esAceituna sinAceitunas pizza
...
cantidadEnPizza :: (Ingrediente -> Int) -> Pizza -> Int
cantidadEnPizza f Prepizza = 0
cantidadEnPizza f (Capa ingrediente pizza) = f ingrediente +  cantidadEnPizza f pizza

cantidadDeCapas :: Pizza -> Int
cantidadDeCapas pizza = cantidadEnPizza (const 1) pizza

cantidadDeAceitunas :: Pizza -> Int
cantidadDeAceitunas pizza = cantidadEnPizza cantSiEsAceituna pizza

appPizza :: (Ingrediente -> Ingrediente) -> Pizza -> Pizza
appPizza f Prepizza = Prepizza
appPizza f (Capa ingrediente pizza) = Capa (f ingrediente) (appPizza f pizza)

duplicarAceitunas :: Pizza -> Pizza 
duplicarAceitunas pizza = appPizza dupSiEsAceituna pizza

-- Hay forma de hacer una sola funcion de returnSomethingAboutPizza y returnBoolAboutPizza
returnSomethingAboutPizza :: (Ingrediente -> Bool) -> (Pizza -> Pizza) -> Pizza -> Pizza
returnSomethingAboutPizza f res Prepizza = Prepizza 
returnSomethingAboutPizza f res (Capa ingrediente pizza) = if f ingrediente
																then res pizza
																else Capa ingrediente (returnSomethingAboutPizza f res pizza) 

sinLactosa :: Pizza -> Pizza
sinLactosa pizza = returnSomethingAboutPizza esQueso sinLactosa pizza

returnBoolAboutPizza :: (Ingrediente -> Bool) -> Bool ->  Bool -> Pizza -> Bool
returnBoolAboutPizza f boolTrueCondition boolBase Prepizza =  boolBase
returnBoolAboutPizza f boolTrueCondition boolBase (Capa ingrediente pizza) = if f ingrediente 
																			then boolTrueCondition
																			else returnBoolAboutPizza f boolTrueCondition boolBase pizza
aptaIntoleranteLactosa :: Pizza -> Bool
aptaIntoleranteLactosa pizza = returnBoolAboutPizza esQueso False True pizza

agregaAceitunasCorrectamente :: Pizza -> Bool
agregaAceitunasCorrectamente pizza = returnBoolAboutPizza esIngredienteInvalido False True pizza

tiene :: Ingrediente -> Pizza -> Bool
tiene ingrediente pizza =  returnBoolAboutPizza (sonIngredientesIguales ingrediente) True False pizza

--Contemplando el caso de que NECESARIMENTE tenga que haber 2 constructores de Aceitunas juntos. 
...
-- Contemplando los casos en que las Aceitunas puedan o no estar todas juntas.
conDescripcionMejorada' :: Pizza -> Pizza
...
juntarPlanillas Fin otraPlanilla = otraPlanilla
...
esMisma :: VariasCosas Tesoro Monstruo -> VariasCosas Tesoro Monstruo -> Bool
esMisma (Objeto x) (Objeto y) =  esMismoTesoro x y
esMisma (Criatura x ) (Criatura y) = esMismoMonstruo x y
esMisma _ _ = False 

esMismoMonstruo ::  Monstruo ->  Monstruo -> Bool
esMismoMonstruo Gargola Gargola = True
esMismoMonstruo Dragon Dragon = True
esMismoMonstruo Troll Troll = True
esMismoMonstruo _ _ = False

esMismoTesoro :: Tesoro -> Tesoro -> Bool
esMismoTesoro Cofre Cofre = True
esMismoTesoro Oro Oro = True
esMismoTesoro Joyas Joyas = True
esMismoTesoro _ _ = False 
...
llenoDe x (Habitacion y) = esMisma x y 
llenoDe x (Pasaje maybe dungeon) =  evaluarMaybeMismaCosa x maybe && llenoDe x dungeon
llenoDe x (Bifurcacion maybe dungeon0 dungeon1) =  evaluarMaybeMismaCosa x maybe && llenoDe x dungeon0 && llenoDe x dungeon1 


equipoPlanilla = Registro "Oswald"
					(juntarPlanillas
						(juntarPlanillas
							(Registro "Alonzo"
								(juntarPlanillas
...
--evaluarMaybeMismaCosa :: a -> Maybe a -> Bool
evaluarMaybeMismaCosa x Nothing = False
evaluarMaybeMismaCosa x (Just y) = esMisma x y